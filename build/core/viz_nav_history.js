...
function captureNavSnapshot() {
  ...
  const svSnap = svActive && typeof window.svGetNavSnapshot === "function" ? window.svGetNavSnapshot() : null;
  return {
...
function _navSnapshotKey(snap) {
  if (!snap) return "";
  const vp = snap.viewport ? `${Math.round(snap.viewport.zoom * 1e3)}:${Math.round(snap.viewport.pan.x)}:${Math.round(snap.viewport.pan.y)}` : "";
  const svZoom = snap.sv && snap.sv.zoom ? `${Math.round(snap.sv.zoom.k * 1e3)}:${Math.round(snap.sv.zoom.x)}:${Math.round(snap.sv.zoom.y)}` : "";
  const sv = snap.sv ? `${snap.sv.fileRel || ""}:${snap.sv.focusId || ""}:${svZoom}` : "";
  return [
    snap.view,
    snap.level,
    snap.activeModule || "",
...
function _applyGraphSnapshotNow(snap) {
  ...
  if (snap.viewport) {
    try {
      cy.viewport({ zoom: snap.viewport.zoom, pan: snap.viewport.pan });
    } catch (_) {
    }
  }
  try {
...
function applyPendingGlobalNavRestore(stage) {
  ...
  const applied = snap.view === "l3" ? typeof window.svApplyNavSnapshot === "function" && window.svApplyNavSnapshot(snap.sv) : _applyGraphSnapshotNow(snap);
  if (applied) {
