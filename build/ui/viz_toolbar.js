...
function updateL1Toolbar(modId, fileCount) {
  ...
  if (labelEl) {
    labelEl.textContent = modId || T("noModule");
    labelEl.title = modId || "";
  }
  const statsEl = document.getElementById("l1-stats");
  if (statsEl) {
    statsEl.dataset.count = String(fileCount || 0);
    statsEl.textContent = T("countFiles", { count: fileCount || 0 });
  }
}
...
function rerenderCurrentL1() {
  ...
  renderFilesFlat(state.activeModule, filtered, state.activeSubDir || void 0);
}
...
function showNodeModal(node) {
  ...
  if (d._t === "file") {
    const pathLine = lines[1] && lines[1].startsWith("\xA7") ? lines[1].slice(1, -1) : null;
    ...
    displaySubtitle = displaySubtitle.replace(/\(External file\)/gi, "").replace(/\(EXTERNAL FILE\)/gi, "").replace(/\(EXTERNAL MODULE\)/gi, "").replace(/<br><br>$/, "").trim();
  ...
  if (d._t === "potential_func") {
    html += `<button class="tip-btn" data-action="open-ambiguous" data-func="${encodeURIComponent(d.fn || "")}">${escapeHtml(T("tooltipOpenLocation"))}</button><button class="tip-btn" data-action="view-ambiguous" data-func="${encodeURIComponent(d.fn || "")}">${escapeHtml(T("tooltipViewFile"))}</button>`;
  } else {
    html += `<button class="tip-btn" data-action="open" data-file="${encodeURIComponent(d._f?.path || d._f || "")}" data-func="${encodeURIComponent(d.fn || "")}" data-node-type="${d._t || ""}" data-mod="${encodeURIComponent(d.mod || "")}">${escapeHtml(T("tooltipOpenLocation"))}</button><button class="tip-btn" data-action="view" data-file="${encodeURIComponent(d._f?.path || d._f || "")}" data-func="${encodeURIComponent(d.fn || "")}">${escapeHtml(T("tooltipViewFile"))}</button>`;
  }
  html += `</div>`;
  html += `</div>`;
  ...
  if (outEdges.length > 0 || inEdges.length > 0) {
    ...
    const OUT_MAP = {
      ...
      "Comp": T("relComponent"),
      "GUID": T("relGuidRef"),
      "Strings": T("relStrings"),
      "ASL": T("relAslInclude"),
      "Callback": T("relCallback"),
      "HII-Pkg": T("relHiiPkg"),
      "Depex": T("relDepex"),
      "Import": T("relImports"),
      "ext": T("relExternalCalls"),
      "group": T("relGroup"),
    ...
    const IN_MAP = {
      ...
      "Src": T("relSourceOf"),
      "Pkg": T("relPackagedIn"),
      "Lib": T("relUsedAsLibBy"),
      "ELINK": T("relElinkParentOf"),
      "Comp": T("relUsedAsCompBy"),
      "GUID": T("relReferencedGuidBy"),
      "Strings": T("relReferencedAsStringBy"),
      "ASL": T("relIncludedByAsl"),
      "Callback": T("relTriggeredBy"),
      "HII-Pkg": T("relPackagedInHii"),
      "Depex": T("relDependedBy"),
      "Import": T("relImportedBy"),
      "ext": T("relExternalCallers"),
      "group": T("relGroup"),
  ...
  content.querySelectorAll(".modal-dep-item").forEach((el) => {
    ...
    el.addEventListener("click", () => {
      ...
      if (targetNode && targetNode.length) {
        hideNodeModal();
        const d2 = targetNode.data();
        if (d2._t === "module") drillToModule(d2._m.id);
        else {
...
function resetL2State(fileRel) {
  ...
  l2State.expandedModules = /* @__PURE__ */ new Set();
  l2State.expandedSysCategories = /* @__PURE__ */ new Set();
...
function focusL2External(entry, opts = {}) {
  ...
  if (!node || !node.length) {
    node = cy.nodes().filter((n) => n.data("_t") === "ext_func" && n.data("fn") === entry.func && n.data("mod") === entry.mod);
  }
  if (node && node.length) {
    cy.elements().unselect();
    node.select();
...
function toggleSysGroup(catName) {
  ...
  if (l2State.expandedSysCategories.has(catName)) {
    ...
    l2State.expandOriginPos = groupNode && groupNode.length ? { ...groupNode.position() } : null;
  }
  l2State.preserveViewport = { pan: { ...cy.pan() }, zoom: cy.zoom() };
  renderL2Flowchart(l2State.activeFile);
}
function _toggleSysGroup_old(catName) {
  ...
  const sysMap = l2State._sysMap || /* @__PURE__ */ new Map();
  const unkMap = l2State._unkMap || /* @__PURE__ */ new Map();
  ...
  if (isExpanded) {
    ...
    if (expandedNodes.length) {
      cx /= expandedNodes.length;
      cy2 /= expandedNodes.length;
    }
...
${funcCount} funcs`,
        ...
        _t: "sys_group",
        syscat: catName,
        tt: isUnk ? `Unresolved symbols (${funcCount})
Click to expand \u2195` : `${catName}
${funcCount} funcs
...
Click to expand \u2195`
    ...
    const callerSet = /* @__PURE__ */ new Map();
    if (isUnk) {
      unkMap.forEach((callers) => callers.forEach((idx) => callerSet.set(idx, (callerSet.get(idx) || 0) + 1)));
    } else {
      (sysMap.get(catName) || /* @__PURE__ */ new Map()).forEach((callerSetI) => callerSetI.forEach((idx) => callerSet.set(idx, (callerSet.get(idx) || 0) + 1)));
    }
    ...
    groupNode.remove();
    const fnMap = isUnk ? unkMap : sysMap.get(catName) || /* @__PURE__ */ new Map();
...
(${catName})`,
          ...
          syscat: catName,
          tt: isUnk ? `${funcName}
Unresolved \u2014 not found in scanned files.` : `${funcName}
${catName}
...
Known system API.`
      ...
      const callers = isUnk ? callerSet : callerSet;
      callers.forEach((callerIdx) => {
        const edgeId = isUnk ? `unkfne-${_hashId(funcName)}-${callerIdx}` : `sysfne-${catSlug}-${callerIdx}-${_hashId(funcName)}`;
        if (!cy.$id(edgeId).length && cy.$id(`fn-${callerIdx}`).length) {
          cy.add({
            data: {
              id: edgeId,
