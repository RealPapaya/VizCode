const _PREFS = {
  KEYS: {
    font: "biosviz_code_font",
    lang: "biosviz_lang",
    extFiles: "biosviz_ext_files",
    extFuncs: "biosviz_ext_funcs",
    theme: "biosviz_theme",
    layoutL0: "biosviz_layout_l0",
    // default layout for L0 module overview
    layoutL1: "biosviz_layout_l1",
    // default layout for L1 dep-map
    layoutL2: "biosviz_layout_l2",
    // default layout for L2 call-flow
    shapeMode: "vc_shape_mode",
    edgeTypeLabels: "biosviz_edge_type_labels",
    svEdgeStyle: "vc_sv_edge_style"
    // Symbol View edge routing: 'bezier' | 'orthogonal'
  },
  DEFAULTS: {
    ...
    theme: "dark",
    layoutL0: "cose",
    // Force-directed — best for module overview
    layoutL1: "dagre-lr",
    // Hierarchy LR — best for dep-map
    layoutL2: "dagre-lr",
    // Hierarchy LR — best for call-flow
    shapeMode: "simple",
  ...
  set(k, v) {
    try {
      localStorage.setItem(this.KEYS[k], String(v));
    } catch (_) {
    }
  },
  load() {
...
function _refreshSearchChrome() {
  ...
  if (filesBtn) {
    filesBtn.setAttribute("data-tip", T("searchModeFilesTip"));
    filesBtn.setAttribute("aria-label", T("searchModeFiles"));
  }
  if (codeBtn) {
    codeBtn.setAttribute("data-tip", T("searchModeCodeTip"));
    codeBtn.setAttribute("aria-label", T("searchModeCode"));
  }
...
const CY_THEME_OVERRIDES = {
  ...
  claude: [
    ...
    {
      selector: "node[simple=1]:selected",
      style: {
        "border-color": "#e8194b"
      }
    },
    {
      selector: "node.selected-label",
      style: {
        "color": "#111827"
      }
    },
...
function _prefAiSyncNavState(cfg) {
  ...
  if (configured) {
    badge.textContent = mode === "cli" ? `CLI: ${cfg?.cli_agent || "claude"}` : _prefAiProviderLabel(cfg?.provider || "anthropic");
  } else {
        ...
        <main class="pref-main">
          ...
          <div class="pref-section-content">
            ...
            <section class="pref-subsection" data-pref-section-panel="maingraph">
              <div class="pref-group-header" data-i18n="mgGroupL0">L0 \xB7 Module Overview</div>
              ...
              `)}
              <div class="pref-group-header" data-i18n="mgGroupL1">L1 \xB7 Dependency Map</div>
              ...
              `)}
              <div class="pref-group-header" data-i18n="mgGroupL2">L2 \xB7 Call Flow</div>
              ...
              ${row("pref-ext-funcs", "extFuncsAlways", "External Functions always ON", `
                ...
                <option value="true" data-i18n="behaviourOn">Always On</option>
              `)}
              <div class="pref-group-header" data-i18n="mgGroupL3">L3 \xB7 Symbol View</div>
...
function _previewLayout(id) {
  ...
  const row = (labelKey, textKey) => `<div style="display:grid;grid-template-columns:80px 1fr;gap:6px 10px;align-items:baseline;">
...
function _previewSvEdgeStyle(value) {
  ...
  const previewLabel = typeof T === "function" ? T(isBezier ? "symbolEdgeStyleBezier" : "symbolEdgeStyleOrthogonal") : isBezier ? "Bezier Curves" : "Orthogonal";
  const nodes = [
...
function _activatePrefSection(sectionId, previewId) {
  ...
  const targetId = previewId || modal.querySelector(`.pref-nav-item[data-pref-section="${selected}"]`)?.dataset.prefPreviewId || activePanel?.querySelector("select")?.id;
...
const _PREF_PREVIEW_SELECT_IDS = /* @__PURE__ */ new Set([
  "font-select",
  "pref-theme-select",
  "pref-lang-select",
  "pref-node-style",
...
function _syncPrefDropdown(sel) {
  ...
  Array.from(sel.options).forEach((opt) => {
    ...
    btn.style.cssText = "width:100%;display:flex;align-items:center;justify-content:space-between;gap:10px;border:none;background:" + (opt.selected ? "color-mix(in srgb, var(--accent) 16%, var(--panel2))" : "transparent") + ";color:" + (opt.selected ? "var(--text)" : "var(--muted)") + ";padding:8px 10px;border-radius:6px;cursor:pointer;font-size:13px;font-family:inherit;text-align:left;transition:background 0.15s ease,color 0.15s ease;";
    ...
    const applyHoverState = (hovered) => {
      ...
      btn.style.background = hovered ? "color-mix(in srgb, var(--accent) 12%, var(--panel2))" : selected ? "color-mix(in srgb, var(--accent) 16%, var(--panel2))" : "transparent";
...
function _previewFont(v) {
  ...
  return `<div style="font-family:${v};font-size:13px;line-height:1.9;padding:14px 16px;background:var(--panel2);border-radius:7px;border:1px solid var(--border);width:100%;box-sizing:border-box;white-space:pre-wrap;word-break:break-all;"><span style="color:#6ab0f5;">const</span> scan = (root) => {<br>&nbsp;&nbsp;<span style="color:#e5c07b;">nodes</span>, <span style="color:#e5c07b;">edges</span> = parse(root);<br>&nbsp;&nbsp;<span style="color:#6ab0f5;">return</span> { nodes, edges };<br>};<br><br><span style="color:var(--muted);font-size:11px;">${name}</span></div>`;
}
function _previewLang(v) {
  ...
  return `<div style="display:flex;flex-direction:column;gap:6px;width:100%;">` + items.map((val) => `<div style="padding:5px 10px;background:var(--panel2);border-radius:5px;border:1px solid var(--border);font-size:13px;color:var(--text);text-align:center;">${val}</div>`).join("") + `</div>`;
}
...
function _previewExtFiles(v) {
  ...
  return `<svg width="230" height="140" viewBox="0 0 230 140" xmlns="http://www.w3.org/2000/svg">
      ...
      ${on ? `<rect x="166" y="28" width="56" height="26" rx="4" fill="var(--muted)" opacity="0.4" stroke="var(--muted)" stroke-width="1" stroke-dasharray="4 2"/>
           <text x="194" y="45" text-anchor="middle" fill="var(--text)" font-size="8" font-family="monospace" opacity="0.65">os.path</text>
           <line x1="144" y1="28" x2="166" y2="38" stroke="var(--muted)" stroke-width="1" stroke-dasharray="3 2" opacity="0.45"/>` : `<text x="194" y="38" text-anchor="middle" fill="var(--muted)" font-size="9" opacity="0.35">&#x2205;</text>`}
...
function _previewExtFuncs(v) {
  ...
  return `<svg width="230" height="140" viewBox="0 0 230 140" xmlns="http://www.w3.org/2000/svg">
      ...
      ${on ? `<circle cx="185" cy="38" r="14" fill="var(--muted)" opacity="0.25" stroke="var(--muted)" stroke-width="1" stroke-dasharray="3 2"/>
           ...
           <line x1="134" y1="90" x2="172" y2="90" stroke="var(--muted)" stroke-width="1" stroke-dasharray="3 2" opacity="0.4"/>` : `<text x="185" y="65" text-anchor="middle" fill="var(--muted)" font-size="9" opacity="0.35">&#x2205;</text>`}
...
function _previewEdgeTypeLabels(v) {
  ...
  return `<svg width="230" height="140" viewBox="0 0 230 140" xmlns="http://www.w3.org/2000/svg">
      ...
      ${on ? `<rect x="92" y="49" width="46" height="18" rx="5" fill="#111827" opacity="0.9"/>
           <text x="115" y="61" text-anchor="middle" fill="#e5e7eb" font-size="8" font-family="monospace">Include</text>` : `<text x="115" y="57" text-anchor="middle" fill="var(--muted)" font-size="10" font-family="monospace" opacity="0.55">&#x2205;</text>`}
...
function initPreferences() {
  ...
  if (fontSel) {
    ...
    fontSel.style.fontFamily = savedFont;
  }
  ...
  prefModal.addEventListener("click", (e) => {
    if (e.target === prefModal) close();
  });
  if (fontSel) fontSel.addEventListener("change", (e) => {
    const f = e.target.value;
    applyFont(f, _PREFS.get("lang"));
    _PREFS.set("font", f);
  ...
  if (svEdgeStyleSel) {
    ...
    svEdgeStyleSel.addEventListener("change", (e) => {
      ...
      if (typeof _svState !== "undefined" && _svState.fileRel && typeof _svLoadFileGraph === "function") {
