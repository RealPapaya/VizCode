...
window._lswUpdate = function(opts = {}) {
  ...
  if (opts.disabled !== void 0) bar.classList.toggle("lsw-disabled", !!opts.disabled);
  const segs = bar.querySelectorAll(".lsw-seg");
  if (opts.l1Available !== void 0) segs[1]?.classList.toggle("lsw-unavailable", !opts.l1Available);
  if (opts.l2Available !== void 0) segs[2]?.classList.toggle("lsw-unavailable", !opts.l2Available);
  if (opts.l3Available !== void 0) segs[3]?.classList.toggle("lsw-unavailable", !opts.l3Available);
...
function _lswSyncLevelAvailability() {
  ...
  const hasSymbols = !!(fileRel && DATA?.symbol_index && Object.values(DATA.symbol_index).some((s) => s.file === fileRel));
  window._lswUpdate({
...
function isRenderedMarkdownSupported(ext, fname, langHint = "") {
  ...
  return ext === ".md" || ext === ".mdx" || langHint === "markdown" || lowerName === "readme" || lowerName === "changelog";
}
...
function syncCodePanelViewToggle(forceVisible = null) {
  ...
  const supported = forceVisible !== null ? !!forceVisible : !!(codeState.currentData && isRenderedMarkdownSupported(codeState.currentExt, codeState.currentName, codeState.currentLangHint));
...
function _cpSyncMultiSnipBtn() {
  ...
  btn.title = codeState.multiSnip ? "Multi-snippet ON \u2014 click to switch back to full file view" : "Multi-snippet mode (Structure View only)";
}
...
async function loadFileInPanel(filePath, funcName) {
  if (!filePath) return;
  if (codeState.userClosed && !codeState.isOpen) return;
  openCodePanel();
...
<div style="display:flex;flex-direction:column;align-items:center;padding:20px;gap:12px;min-height:200px">
  ...
  />
  <div style="font-size:11px;color:var(--muted);font-family:var(--code-font)">${escapeHtml(data.path || "")} &nbsp;\xB7&nbsp; ${escapeHtml(kb)}</div>
</div>`;
  wrap.style.display = "";
...
function renderHexDump(data) {
  ...
  const kb = data.size ? (data.size / 1024).toFixed(1) + " KB" : "";
  const trunc = data.truncated ? `<div style="color:#f59e0b;font-size:11px;padding:8px 0">\u26A0 Showing first 8 KB of ${escapeHtml(kb)} file</div>` : "";
  const rows = lines.map((ln) => {
    ...
    return `<div class="hex-row"><span class="hex-addr">${escapeHtml(addr || "")}</span><span class="hex-bytes">${escapeHtml(hexPart)}</span><span class="hex-ascii">${escapeHtml(asciiPart)}</span></div>`;
  }).join("");
  wrap.innerHTML = `
...
function renderCode(src, ext, fname, langHint) {
  ...
  const lang = langHint && langHint !== "plaintext" ? langHint : hlExt[ext] || hlFilename[fname] || "plaintext";
  codeState.funcLineMap = {};
  codeState.funcList = [];
  ...
  const wrap = document.getElementById("cp-code-wrap");
  const lineDivs = highlightedLines.map(
    (hl, i) => `<div class="code-line" id="cl-${i}"><span class="line-num">${i + 1}</span><span class="line-content">${hl}</span></div>`
  ).join("");
  wrap.innerHTML = `<pre><code class="hljs language-${lang}">${lineDivs}</code></pre>`;
  wrap.style.display = "";
  wrap.onclick = (e) => {
    const lineEl = e.target.closest(".code-line");
    const clickedLine = lineEl && lineEl.id.startsWith("cl-") ? parseInt(lineEl.id.slice(3), 10) : -1;
...
function jumpToLine(lineNo) {
  const idx = lineNo - 1;
  document.querySelectorAll(".code-line.fn-highlight").forEach((el2) => el2.classList.remove("fn-highlight"));
