...
function _gtipOver(e) {
  ...
  if (raw) {
    raw.dataset.tip = raw.getAttribute("title");
...
function _gtipShow(target, e) {
  ...
  requestAnimationFrame(() => {
    _gtipPos(e.clientX, e.clientY);
    _gtip.el.classList.add("g-tip-visible");
  });
}
function _gtipHide() {
  clearTimeout(_gtip.timer);
...
async function _jobViewerOpen(forceNew = false) {
  ...
  _jobViewerLease.openPromise = (async () => {
    ...
    if (Number.isFinite(data.ping_interval_seconds) && data.ping_interval_seconds > 0) {
      _jobViewerLease.pingMs = Math.max(5e3, data.ping_interval_seconds * 1e3);
    }
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      try {
        ...
        if ((rootOther.length || rootFiles.length) && Array.isArray(DATA.modules) && !DATA.modules.some((m) => m.id === "_root")) {
...
function onKey(e) {
  ...
  if (e.key === "c" || e.key === "C") {
    document.getElementById("code-toggle-btn").click();
  }
  if (e.key === "g" || e.key === "G") {
...
function onNodeRightClick(ev, node) {
  ...
  document.getElementById("ctx-pin").onclick = () => {
    const id = node.id();
    if (state.pinnedNodes.has(id)) {
      state.pinnedNodes.delete(id);
      node.unlock();
    } else {
...
function showTooltip(e) {
  ...
  if (e.target.isNode()) {
    ...
    if (d._t === "ext_func") {
      ...
      html += fileRel ? `<div class="tip-body">${escapeHtml(fileRel)}</div>` : `<div class="tip-body">${escapeHtml(T("tooltipUnknownTarget"))}</div>`;
      html += `<div class="tip-actions"><button class="tip-btn" data-action="open" data-file="${encodeURIComponent(fileRel)}" data-func="${encodeURIComponent(funcName)}">${escapeHtml(T("tooltipOpenLocation"))}</button><button class="tip-btn" data-action="view" data-file="${encodeURIComponent(fileRel)}" data-func="${encodeURIComponent(funcName)}">${escapeHtml(T("tooltipViewFile"))}</button></div>`;
    } else {
      const lines = d.tt ? d.tt.split("\n") : [];
      const titleRaw = lines[0] || "";
      const pathMatch = lines[1] && lines[1].startsWith("\xA7") ? lines[1].slice(1, -1) : null;
    ...
    const kindBadge = d.kind ? `<span style="font-size:10px;color:#94a3b8;margin-left:6px;opacity:0.8">[${escapeHtml(d.kind)}]</span>` : "";
  ...
  const fullHtml = briefName ? `<div class="tip-brief">${escapeHtml(briefName)}</div><div class="tip-full">${html}</div>` : html;
