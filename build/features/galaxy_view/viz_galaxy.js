...
let _gHighlightSet = /* @__PURE__ */ new Set();
let _gBlastSet = /* @__PURE__ */ new Set();
let _gAnimatedNodes = /* @__PURE__ */ new Map();
...
let _gHideEdgesDuringLayout = false;
let _gBackgroundHooksInstalled = false;
let _gLastUserActionAt = 0;
let _gLayoutDone = false;
let _gDataFingerprint = null;
let _gAllowedMods = null;
let _gSavedAllowedMods = null;
let _gDegreeCache = null;
let _gNodeLabelCache = null;
let _gSearchLower = "";
...
const _G_CLASS_KINDS = /* @__PURE__ */ new Set(["class", "struct", "interface", "enum", "typedef", "trait", "protocol", "mixin", "module", "impl"]);
const _G_FUNCTION_KINDS = /* @__PURE__ */ new Set(["function"]);
const _G_METHOD_KINDS = /* @__PURE__ */ new Set(["method"]);
const _G_CODE_NODE_TYPES = /* @__PURE__ */ new Set(["class", "struct", "interface", "enum", "typedef", "trait", "protocol", "mixin", "module", "impl", "function", "method"]);
const _G_NODE_THRESHOLD = 5e3;
const _galaxyFilter = {
  nodeTypes: /* @__PURE__ */ new Set(["folder", "file", "class", "struct", "interface", "enum", "typedef", "trait", "protocol", "mixin", "module", "impl", "function", "method"]),
  edgeTypes: /* @__PURE__ */ new Set(["contain", "import", "call", "define", "extend", "implements", "mixin_include", "mixin_extend", "mixin_prepend", "behaviour_impl", "protocol_impl", "override"]),
  searchQuery: "",
  minDegree: 0,
  depthHops: 0
};
...
function _galaxyShouldShowButtonEffect() {
  return !!(_gPrecomputePending || _gPrecomputeQueued || _gBackgroundPrecomputeMode || _gLayoutRunning || state?.galaxyActive && _gLayoutNeedsNoverlap);
}
...
function _gUpdateDepthFilterState() {
  ...
  if (!hasPinned) {
    ...
    depthLabel.innerHTML = depthH === 0 ? "Off \u2014 show all nodes" : `<strong class="gf-deg-val">${depthH}</strong> hop${depthH !== 1 ? "s" : ""} from pinned`;
  }
}
function _gCodeLocation(attrs) {
  if (!attrs._file || !attrs._line) return "-";
  return attrs._endLine && attrs._endLine > attrs._line ? `${attrs._file}:${attrs._line}-${attrs._endLine}` : `${attrs._file}:${attrs._line}`;
}
...
const _G_LAYOUT_STORAGE_VERSION = 2;
const _G_LAYOUT_STORAGE_LIMIT_BYTES = 6 * 1024 * 1024;
...
function _galaxyLayoutStorageLoad() {
  ...
  try {
    ...
    console.log(`[galaxy] restored layout from localStorage (${applied}/${_gGraph.order} nodes, age ${((Date.now() - parsed.ts) / 1e3).toFixed(0)}s)`);
    return true;
  } catch (err) {
...
function _galaxyBuildFilterPanel() {
  ...
  const hdr = (label, actions = "") => `<div class="flt-section-hdr" style="${actions ? "" : "pointer-events:none"}"><span class="flt-section-label">${label}</span>` + (actions ? `<span class="flt-actions">${actions}</span>` : "") + `</div>`;
  const row = (key, label, color, icon, dataAttr, activeSet) => `<div class="nl-row gf-row${activeSet.has(key) ? " active" : ""}" ${dataAttr}="${key}" style="--nl-col:${color};cursor:pointer"><div class="nl-icon-bg"><span class="nl-shape" style="color:${color}">${icon}</span></div><span class="nl-label" style="color:${color}">${label}</span></div>`;
  const nodeDefs = _galaxyPresentNodeDefs();
  const edgeDefs = _galaxyPresentEdgeDefs();
  const nodeRows = nodeDefs.map((def) => row(def.key, def.label, def.color, def.icon, "data-gf-node", _galaxyFilter.nodeTypes)).join("");
  const edgeRows = edgeDefs.map(
    (def) => `<div class="ef-row gf-row${_galaxyFilter.edgeTypes.has(def.key) ? " active" : ""}" data-gf-edge="${def.key}" style="--ef-col:${def.color}"><div class="ef-icon-bg"><span class="ef-indicator"><svg width="28" height="10" viewBox="0 0 28 10"><line x1="2" y1="5" x2="26" y2="5" stroke="${def.color}" stroke-width="2" stroke-linecap="round"></line></svg></span></div><span class="ef-label">${def.label}</span></div>`
  ).join("");
  const ntActions = `<button class="flt-action is-add" data-gf-node-action="all" title="${typeof T === "function" ? T("selectAll") : "All"}">+</button><button class="flt-action is-rm" data-gf-node-action="none" title="${typeof T === "function" ? T("selectNone") : "None"}">\u2212</button>`;
  const etActions = `<button class="flt-action is-add" data-gf-edge-action="all" title="${typeof T === "function" ? T("selectAll") : "All"}">+</button><button class="flt-action is-rm" data-gf-edge-action="none" title="${typeof T === "function" ? T("selectNone") : "None"}">\u2212</button>`;
  const minDeg = _galaxyFilter.minDegree;
  const depthH = _galaxyFilter.depthHops;
  const depthDisabled = !_gPinned;
  wrap.innerHTML = `<div id="filters-title" data-i18n="filters">${typeof T === "function" ? T("filters") : "Filters"}</div>${hdr("Node Types", ntActions)}<div style="padding:4px 0 8px">${nodeRows}</div>${hdr("Edge Types", etActions)}<div style="padding:4px 0 8px">${edgeRows}</div>${hdr("Min Connections")}<div style="padding:4px 0 8px"><div class="gf-slider-wrap"><input type="range" class="gf-slider" id="gf-mindeg-slider" min="0" max="20" step="1" value="${minDeg}"><div class="gf-slider-label">Show nodes with <strong class="gf-deg-val" id="gf-deg-val">${minDeg}</strong> connection${minDeg !== 1 ? "s" : ""}</div></div></div>${hdr("Depth Filter")}<div style="padding:4px 0 8px"><div class="gf-slider-wrap${depthDisabled ? " disabled" : ""}"><input type="range" class="gf-slider" id="gf-depth-slider" min="0" max="5" step="1" value="${depthH}"${depthDisabled ? " disabled" : ""}><div class="gf-slider-label">${depthDisabled ? '<em style="color:#475569">Pin a node to enable</em>' : depthH === 0 ? "Off \u2014 show all nodes" : `<strong class="gf-deg-val" id="gf-depth-val">${depthH}</strong> hop${depthH !== 1 ? "s" : ""} from pinned`}</div></div></div>${hdr("Display")}<div style="padding:4px 0 8px"><div class="nl-row gf-row${_gCommunityColors ? " active" : ""}" id="gf-community-toggle" style="--nl-col:#8b5cf6;cursor:pointer"><div class="nl-icon-bg"><span class="nl-shape" style="color:#8b5cf6">&#9673;</span></div><span class="nl-label" style="color:#8b5cf6">Community Colors</span></div></div>`;
  ...
  wrap.querySelectorAll("[data-gf-node-action]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      if (action === "all") {
        ...
        wrap.querySelectorAll("[data-gf-node]").forEach((row2) => row2.classList.add("active"));
      } else if (action === "none") {
        _galaxyFilter.nodeTypes.clear();
        wrap.querySelectorAll("[data-gf-node]").forEach((row2) => row2.classList.remove("active"));
      }
      if (_gSig) _gSig.refresh();
    });
  });
  wrap.querySelectorAll("[data-gf-edge-action]").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      if (action === "all") {
        ...
        wrap.querySelectorAll("[data-gf-edge]").forEach((row2) => row2.classList.add("active"));
      } else if (action === "none") {
        _galaxyFilter.edgeTypes.clear();
        wrap.querySelectorAll("[data-gf-edge]").forEach((row2) => row2.classList.remove("active"));
      }
      if (_gSig) _gSig.refresh();
    });
  });
  ...
  if (depthSlider) {
    depthSlider.addEventListener("input", () => {
      ...
      if (depthLabel) {
        depthLabel.innerHTML = _galaxyFilter.depthHops === 0 ? "Off \u2014 show all nodes" : `<strong class="gf-deg-val">${_galaxyFilter.depthHops}</strong> hop${_galaxyFilter.depthHops !== 1 ? "s" : ""} from pinned`;
      }
      if (_gSig) _gSig.refresh();
    });
  }
...
function _galaxyShowTopbarSearch() {
  ...
  const applySearch = _gDebounce((value) => {
    ...
    _gSearchLower = (value || "").toLowerCase().trim();
    if (_gSig) _gSig.refresh();
  }, 120);
...
function _galaxyShowChangeFoldersBtn() {
  ...
  btn.style.cssText = "position:absolute;top:14px;left:14px;z-index:20;display:flex;align-items:center;gap:7px;padding:6px 13px 6px 10px;background:rgba(10,20,35,0.82);border:1px solid rgba(100,150,255,0.25);border-radius:6px;cursor:pointer;font-size:12px;font-weight:500;color:#94a3b8;transition:background 0.15s,border-color 0.15s,color 0.15s;font-family:inherit;";
  btn.innerHTML = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/><line x1="12" y1="11" x2="12" y2="17"/><line x1="9" y1="14" x2="15" y2="14"/></svg><span>Change Folders\u2026</span>`;
  ...
  btn.addEventListener("click", () => {
    ...
    _galaxyShowFolderSelectTree(estTotal, pathTree, (selectedPaths) => {
      ...
      _gSavedAllowedMods = selectedPaths;
      _gLayoutToken++;
...
async function openGalaxy() {
  ...
  if (estTotal > _G_NODE_THRESHOLD && _gAllowedMods === null) {
    ...
    const layoutSwitcher2 = document.getElementById("layout-switcher");
    if (layoutSwitcher2) layoutSwitcher2.style.display = "none";
...
window.zoomGalaxyByStep = function(direction) {
  ...
  const newRatio = Math.max(2e-3, Math.min(50, camera.ratio * factor));
  camera.animate(
    { ratio: newRatio },
...
function _galaxyTeardownSigma() {
  ...
  _G_COMMUNITY_DIM = null;
  _G_COMMUNITY_FOG = null;
  _G_COMMUNITY_HLDIM = null;
  _G_COMMUNITY_SEARCHDIM = null;
  _G_COMMUNITY_BRIGHT = null;
  _gPinned = null;
  _gNeighborSet = null;
  _gHoveredNode = null;
  _gHopSet = null;
...
async function _galaxyLayoutAsync() {
  ...
  runPromise = (async () => {
    try {
      if (shouldRunFA2) {
        _gHideEdgesDuringLayout = !!(_gGraph && _gGraph.size > 6e4);
        let ranWorker = false;
        if (typeof Worker !== "undefined" && _gGraph && _gGraph.order >= 400 && typeof _galaxyFA2RunWorker === "function") {
          try {
        ...
        if (_gLayoutToken !== myToken) return;
        if (!ranWorker) {
...
function _galaxyShowLayoutBadge() {
  ...
  el.style.cssText = "position:absolute;bottom:14px;left:14px;z-index:20;background:rgba(10,20,35,0.82);border:1px solid rgba(100,150,255,0.25);border-radius:6px;padding:5px 10px;font-size:11px;color:#94a3b8;display:flex;align-items:center;gap:7px;pointer-events:none;";
  el.innerHTML = '<span style="display:inline-block;width:7px;height:7px;border-radius:50%;border:1.5px solid #3b82f6;border-top-color:transparent;animation:spin .7s linear infinite"></span>Computing layout\u2026';
...
function _gBuildPathTree(data) {
  ...
  const root = { name: "", path: "", files: 0, symbols: 0, total: 0, children: /* @__PURE__ */ new Map() };
  ...
  function ensure(path) {
    ...
    const n = { name, path, files: 0, symbols: 0, total: 0, children: /* @__PURE__ */ new Map() };
...
function _galaxyShowFolderSelectTree(totalCount, tree, onConfirm) {
  ...
  function setSubtreeChecked(node, on) {
    if (on) checkedPaths.add(node.path);
    else checkedPaths.delete(node.path);
  ...
  const cbByPath = /* @__PURE__ */ new Map();
  const exByPath = /* @__PURE__ */ new Map();
  function buildRow(node, depth) {
    const hasKids = node.children.size > 0;
    const wrapper = document.createElement("div");
    const row = document.createElement("div");
    row.style.cssText = `display:flex;align-items:center;gap:5px;padding:4px 6px 4px ${depth * 14 + 4}px;border-radius:5px;transition:background 0.12s;user-select:none;cursor:pointer`;
    row.onmouseenter = () => {
    ...
    chev.style.cssText = `width:12px;text-align:center;flex-shrink:0;font-size:9px;color:${C.dim};transition:transform 0.15s;line-height:1`;
    ...
    label.style.cssText = `flex:1;font-size:13px;color:${C.text};overflow:hidden;text-overflow:ellipsis;white-space:nowrap`;
    const cnt = document.createElement("span");
    cnt.style.cssText = `font-size:11px;color:${C.muted};white-space:nowrap;flex-shrink:0`;
    cnt.textContent = `${fmt(node.files)} files \xB7 ${fmt(node.symbols)} sym`;
  ...
  overlay.style.cssText = `position:absolute;inset:0;z-index:20;background:${C.ovBg};display:flex;align-items:center;justify-content:center`;
  const dialog = document.createElement("div");
  dialog.style.cssText = `background:${C.dlgBg};border:1px solid ${C.border};border-radius:12px;padding:20px 20px 16px;width:540px;max-width:94vw;max-height:90vh;display:flex;flex-direction:column;box-shadow:0 24px 64px rgba(0,0,0,0.85)`;
  const hdr = document.createElement("div");
  hdr.style.marginBottom = "14px";
  hdr.innerHTML = `<div style="font-size:15px;font-weight:600;color:${C.text};margin-bottom:5px">Codebase too large <span style="font-weight:400;color:${C.dim}">(\u2248 ${fmtFull(totalCount)} nodes)</span></div><div style="font-size:12px;color:${C.dim};line-height:1.5">Galaxy renders best under ${fmtFull(_G_NODE_THRESHOLD)} nodes. Navigate the folder tree and check what to include.</div>`;
  const toolbar = document.createElement("div");
  toolbar.style.cssText = "display:flex;align-items:center;justify-content:space-between;margin-bottom:6px;flex-shrink:0";
  function mkTextBtn(txt) {
    const b = document.createElement("button");
    b.textContent = txt;
    b.style.cssText = `font-size:11px;color:${C.dim};background:none;border:none;cursor:pointer;padding:2px 7px;border-radius:4px;transition:color 0.1s`;
    b.onmouseenter = () => {
  ...
  treeEl.style.cssText = `flex:1;overflow-y:auto;background:${C.listBg};border:1px solid ${C.listBdr};border-radius:7px;padding:4px;min-height:100px;max-height:340px`;
  ...
  footer.style.cssText = "display:flex;align-items:center;justify-content:space-between;margin-top:12px;margin-bottom:14px;flex-shrink:0;min-height:20px";
  ...
  loadAllBtn.style.cssText = `font-size:12px;color:${C.dim};background:none;border:1px solid ${C.btnBdr};border-radius:6px;padding:6px 14px;cursor:pointer;transition:border-color 0.15s,color 0.15s`;
  ...
  loadBtn.style.cssText = "font-size:13px;font-weight:600;color:#fff;background:#4f46e5;border:none;border-radius:6px;padding:7px 18px;cursor:pointer;transition:background 0.15s";
  loadBtn.onmouseenter = () => {
    loadBtn.style.background = "#4338ca";
  };
  loadBtn.onmouseleave = () => {
    loadBtn.style.background = "#4f46e5";
  };
  ...
  function updateFooter() {
    ...
    const clr = total <= _G_NODE_THRESHOLD ? "#10b981" : total <= _G_NODE_THRESHOLD * 1.6 ? "#f59e0b" : "#ef4444";
    countEl.innerHTML = `<span style="color:${C.dim}">Selected: </span><strong style="color:${clr}">${fmtFull(total)}</strong><span style="color:${C.dim}"> nodes</span>`;
  ...
  function _showLargeFolderConfirm(nodeCount, onProceed, onCancel) {
    ...
    confirmOverlay.style.cssText = "position:absolute;inset:0;z-index:30;background:rgba(0,0,0,0.72);display:flex;align-items:center;justify-content:center";
    const box = document.createElement("div");
    box.style.cssText = `background:${C.dlgBg};border:1px solid #3a2020;border-radius:10px;padding:22px 24px 18px;width:360px;max-width:92vw;box-shadow:0 16px 48px rgba(0,0,0,0.9);text-align:center`;
    ...
    body.innerHTML = `You selected <strong style="color:${C.text}">${nodeCount.toLocaleString()} nodes</strong> \u2014 <strong style="color:#f87171">${overRatio}\xD7</strong> the recommended limit of ${_G_NODE_THRESHOLD.toLocaleString()}.<br><br>Galaxy may be <strong style="color:#fbbf24">very slow or unresponsive</strong>.<br>Consider selecting fewer folders.`;
    const btnRow2 = document.createElement("div");
    btnRow2.style.cssText = "display:flex;gap:10px;justify-content:center";
    const cancelBtn2 = document.createElement("button");
    cancelBtn2.textContent = "Go Back";
    cancelBtn2.style.cssText = `font-size:12px;color:${C.dim};background:none;border:1px solid ${C.btnBdr};border-radius:6px;padding:7px 18px;cursor:pointer;transition:border-color 0.15s,color 0.15s`;
    cancelBtn2.onmouseenter = () => {
      cancelBtn2.style.borderColor = C.dim;
      cancelBtn2.style.color = C.text;
    };
    cancelBtn2.onmouseleave = () => {
      cancelBtn2.style.borderColor = C.btnBdr;
      cancelBtn2.style.color = C.dim;
    };
    ...
    proceedBtn.style.cssText = "font-size:13px;font-weight:600;color:#fff;background:#b91c1c;border:none;border-radius:6px;padding:7px 18px;cursor:pointer;transition:background 0.15s";
    proceedBtn.onmouseenter = () => {
      proceedBtn.style.background = "#991b1b";
    };
  ...
  loadBtn.addEventListener("click", () => {
    const total = countSelected();
    if (total > _G_NODE_THRESHOLD) {
      _showLargeFolderConfirm(total, _doLoad, () => {
      });
    } else {
      _doLoad();
    }
...
function scheduleGalaxyPrecompute() {
  ...
  if (_gEstimateTotalNodes(window.DATA) > _G_NODE_THRESHOLD) return;
...
function _galaxyInitSigma() {
  ...
  _gSig.on("enterNode", ({ node }) => {
    _gHoveredNode = node;
    _gHoverNeighborSet = /* @__PURE__ */ new Set();
...
function _galaxyNodeReducer(node, data) {
  ...
  if (anim) {
    ...
    if (anim.type === "pulse") {
      ...
      data.color = phase > 0.5 ? "#06b6d4" : "#29d4f5";
...
window.galaxyClearHighlight = function() {
  _gHighlightSet = /* @__PURE__ */ new Set();
  _gBlastSet = /* @__PURE__ */ new Set();
  _gAnimatedNodes = /* @__PURE__ */ new Map();
  if (_gSig) _gSig.refresh();
};
window.galaxyAnimateNodes = function(nodeKeys, type, duration) {
  const now = Date.now();
  const dur = duration || (type === "ripple" ? 3e3 : 2e3);
...
function _buildGNeighborSet(node) {
  _gNeighborSet = /* @__PURE__ */ new Set();
