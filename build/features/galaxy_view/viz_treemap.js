...
let _overviewTreemapCellsByPath = /* @__PURE__ */ new Map();
...
function _overviewTreemapBuildLayout(files, w, h) {
  ...
  const groups = Array.from(groupsByKey.values()).filter((group) => group.value > 0 && group.files.length).sort((a, b) => {
    const pa = _overviewTreemapGroupPriority(a);
    const pb = _overviewTreemapGroupPriority(b);
    return pa === pb ? b.value - a.value : pa - pb;
  });
  groups.forEach((group, index) => {
    group.color = _overviewTreemapGroupColor(group, index);
  });
  ...
  groupRects.forEach((group) => {
    ...
    const fileItems = group.files.map((item) => ({ file: item.file, value: item.value, color: group.color, group: group.label })).sort((a, b) => b.value - a.value);
...
function _overviewTreemapInstallInteractions(grid) {
  ...
  const finishDrag = (event) => {
    ...
    if (drag.moved) {
      ...
      setTimeout(() => {
        _overviewTreemapSuppressClick = false;
      }, 0);
    }
  };
...
function _overviewTreemapTooltip(cell, event) {
  ...
  _gTooltipEl.innerHTML = `<div class="gt-name" style="color:${cell.color}">${_gEsc(f.label || _gLastSegment(f.path))}</div><div class="gt-type">File</div><div class="gt-loc">${_gEsc(f.path || "")}</div><div class="gt-mod">${_gEsc(cell.group || _gDirname(f.path) || _gRootLabel())}</div><div class="gt-degree">${lines ? `${lines} lines` : _overviewTreemapFmtBytes(f.size)} &middot; ${funcs} functions</div>`;
...
function _overviewRenderTreemap() {
  ...
  _overviewTreemapCellsByPath = /* @__PURE__ */ new Map();
...
function _overviewTreemapDestroy() {
  ...
  if (_overviewTreemapResizeObserver) {
    try {
      _overviewTreemapResizeObserver.disconnect();
    } catch (_) {
    }
