...
<div class="dash-detail-head">
  ...
  ${descTip}
  <button class="dash-detail-close" id="dash-detail-close" type="button" aria-label="Close">\xD7</button>
</div>
<div class="dash-detail-body dash-report-body dash-report-body--${widgetId}" id="dash-detail-body"></div>`;
  ...
  panel.querySelector("#dash-detail-close")?.addEventListener("click", () => _dashCloseDetailPanel());
...
function _dashReportSection({ title, subtitle, accent, body, className } = {}) {
  ...
  const subtitleHTML = subtitle && String(subtitle).includes("<") ? String(subtitle) : subtitle ? _dashEscape(subtitle) : "";
...
function _dashDetailExistingTargets(body) {
  return [...body.querySelectorAll('[data-clickable="true"], [onclick], a[href], button')].filter(
    (el) => el && !el.closest(".dash-detail-head") && !el.closest(".dash-chart-toggle")
  ).map((el) => ({
    el,
...
function _dashDetailNormalize(text) {
  return String(text || "").toLowerCase().replace(/\s+/g, " ").replace(/[^\p{L}\p{N}.+#/\- ]+/gu, "").trim();
}
...
function _dashDetailMetricAction(widgetId, label, value, stats) {
  ...
  const allFunctions = () => {
    const fromIndex = Object.values(window.DATA && DATA.symbol_index || {}).filter((sym) => sym && (sym.kind === "function" || sym.kind === "method")).map((sym) => ({ file: sym.file, name: sym.name, line: sym.line, value: sym.complexity != null ? `cx ${sym.complexity}` : "" }));
...
function _dashDetailReportModel(widgetId, widget, stats) {
  ...
  const extCount = _dashReportFileExts(1e3).length;
  ...
  if (widgetId === "code_health") {
    ...
    const metric = (key, label) => {
      ...
      return { label, value: value.toFixed(1), raw: value, color: typeof _dashHealthColor === "function" ? _dashHealthColor(value) : void 0 };
    };
    const metrics = [
    ...
    return Object.assign(base, {
      ...
      metrics,
      visual: typeof _dashHealthGaugeSvg === "function" ? _dashHealthGaugeSvg(score, color, { scoreFont: 46, denFont: 16, denDy: 24, style: "width:min(360px,100%);height:auto;" }) : _dashReportBarsHTML(metrics)
    });
  }
  ...
  if (widgetId === "kpi_functions") {
    const modCount = Object.values((window.DATA || {}).files_by_module || {}).filter((list) => (list || []).some((f) => (f.func_count || (f.functions || []).length) > 0)).length;
    return Object.assign(base, {
      title: "Functions",
