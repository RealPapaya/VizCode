...
function _dashRenderTabBar() {
  ...
  strip.innerHTML = state.tabs.map((tab) => {
    ...
    const renameBtn = !locked && editActive ? `<button class="dash-tab-rename" type="button" aria-label="Rename ${_dashEscape(tab.name)}">
    ...
    const closeBtn = !locked && editActive ? '<button class="dash-tab-close" type="button" aria-label="Remove tab">x</button>' : "";
...
function _dashGetActiveTabConfig() {
  return {};
}
