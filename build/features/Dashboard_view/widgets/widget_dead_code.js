...
_dashRegisterWidget({
  ...
  render(container, size, stats) {
    ...
    if (size === "S") {
      ...
      (stats.dead_code_symbols || []).forEach((sym) => byFile2.set(sym.file, (byFile2.get(sym.file) || 0) + 1));
      const pills = [...byFile2.entries()].sort((a, b) => b[1] - a[1]).slice(0, 3).map(([file, cnt]) => ({
...
<div class="dash-kpi-l">
  ...
  renderDetail(container, stats) {
    ...
    const summaryText = count === 0 ? "No dead code detected." : `${count} unused symbol${count !== 1 ? "s" : ""} across ${affectedFiles} file${affectedFiles !== 1 ? "s" : ""}.`;
