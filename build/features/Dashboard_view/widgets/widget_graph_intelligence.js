...
function _dashGraphIntelRenderDetail(container, stats) {
  ...
  const summaryText = hotspots.length === 0 ? "No hotspot symbols detected." : `${hotspots.length} hotspot symbol${hotspots.length !== 1 ? "s" : ""}` + (surprising.length > 0 ? ` \xB7 ${surprising.length} surprising connection${surprising.length !== 1 ? "s" : ""}.` : ".");
...
<div style="height:100%;box-sizing:border-box;display:flex;flex-direction:column;gap:var(--space-2);overflow:hidden;">
  ...
  renderDetail(container, stats) {
    _dashGraphIntelRenderDetail(container, stats);
  }
});
