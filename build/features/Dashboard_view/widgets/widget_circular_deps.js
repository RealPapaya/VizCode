...
function _dashCircChainFlowHTML(cycle, expanded, idx) {
  ...
  visible.forEach((file, i) => {
    ...
    const fileJson = _dashJson(file);
    parts.push(
      `<span class="dash-circ-chip" title="${_dashEscape(file)}" onclick="event.stopPropagation();_dashGoToGraphFile(${fileJson},null)">${_dashEscape(short)}</span>`
    );
  ...
  if (!expanded && total > visible.length) {
    parts.push('<span class="dash-circ-arrow" aria-hidden="true"></span>');
    parts.push(
      `<span class="dash-circ-more" onclick="event.stopPropagation();_dashCircDetailToggle(${idx})">+${total - visible.length} more</span>`
    );
  }
  parts.push(
...
function _dashCircCardDetail(cycle, idx) {
  ...
  const titleJson = _dashJson(`Circular #${idx + 1} (${len} files)`);
  const sevLabel = _dashT(sev.labelKey);
  const expandBtn = len > 4 ? `<button class="dash-circ-toggle-btn" data-circ-d-toggle="${idx}" title="${_dashEscape(_dashT("dashIssuesExpandChain"))}"
...
function _dashRenderCircularDeps(container, stats, opts) {
  ...
  const cyclesHTML = cycles.length ? cycles.map((c, i) => _dashCircCard(c, i)).join("") : `<div class="dash-empty">\u2705 ${_dashEscape(_dashT("dashIssuesNoCycles"))}</div>`;
  container.innerHTML = `
    ...
    if (size === "S") {
      const pills = cycles.slice(0, 3).map((c, i) => {
        ...
        return {
          label: `${c.length}f`,
          value: "",
          title: `#${i + 1} \xB7 ${_dashCircSummary(c)}`,
...
<div class="dash-kpi-s">
  <div class="dash-kpi-s-body">
    ...
    if (size === "M") {
      const cyclesHTML = cycles.length ? cycles.slice(0, 3).map((c, i) => _dashCircCard(c, i)).join("") : `<div class="dash-empty">\u2705 ${_dashEscape(_dashT("dashIssuesNoCycles"))}</div>`;
      container.innerHTML = `
<div class="dash-arch-panel">
  <div class="dash-arch-panel-header">
    <div class="dash-arch-panel-title-block">
  ...
  renderDetail(container, stats) {
    ...
    cycles.forEach((c) => {
      sevCounts[_dashCircSeverity(c.length).tier]++;
    });
...
<div class="dash-circ-detail-sev">
  ...
  <div class="dash-circ-detail-sev__item">
    ...
    const summaryText = count === 0 ? "No circular dependencies detected." : `${count} cycle${count !== 1 ? "s" : ""} detected across ${affectedFiles} file${affectedFiles !== 1 ? "s" : ""}.`;
    const cyclesHTML = cycles.length ? cycles.map((c, i) => _dashCircCardDetail(c, i)).join("") : `<div class="dash-empty">\u2705 ${_dashEscape(_dashT("dashIssuesNoCycles"))}</div>`;
    container.innerHTML = `
...
${_dashKpiDetailSectionHTML({
      ...
      body: _dashKpiDetailStatsHTML([
        ...
        { value: String(sevCounts.critical), label: "critical", color: sevCounts.critical > 0 ? "#d35454" : void 0 },
        { value: String(sevCounts.warn), label: "warning", color: sevCounts.warn > 0 ? "var(--status-warn)" : void 0 }
      ])
    })}
${_dashKpiDetailSectionHTML({
