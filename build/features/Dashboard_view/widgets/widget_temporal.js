...
function _dashRenderTemporal(container, stats, size, scope) {
  ...
  const recentCutoff = /* @__PURE__ */ new Date();
  ...
  _dashTemporalDays = latestActivity && latestActivity < recentCutoffDate ? 0 : Math.min(Number(stats.window_days || 180), 180);
  if (size === "S") {
...
<div class="dash-grid dash-grid-2" style="height:100%;box-sizing:border-box;">
  ...
  container.querySelectorAll(".dash-temporal-pill").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      container.querySelectorAll(".dash-temporal-pill").forEach(
...
function _dashTemporalShell(stats, scope) {
  ...
  const pills = [30, 90, 180].filter((d) => d <= maxDays).concat(0).map((d) => {
...
_dashRegisterWidget({
  ...
  render(container, size, stats) {
    _dashRenderTemporal(container, stats, size);
  },
  renderDetail(container, stats) {
    _dashRenderTemporal(container, stats, "L", "detail");
  }
});
