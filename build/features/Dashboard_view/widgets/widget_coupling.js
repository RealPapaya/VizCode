...
<div class="dash-arch-panel">
  <div class="dash-arch-panel-header">
    <div class="dash-arch-panel-title-block">
      ...
      <div class="dash-arch-panel-sub">${imported.length} hotspots \xB7 ${conc}% ${_dashEscape(_dashT("dashCouplingConcentration"))}</div>
    </div>
...
<div class="dash-coupling-detail-conc">
  ...
  const summaryText = imported.length === 0 ? "No dependency hotspots detected." : `${imported.length} hotspot file${imported.length !== 1 ? "s" : ""} \u2014 top 3 account for ${conc}% of all imports.`;
...
<div class="dash-kpi-s">
  <div class="dash-kpi-s-body">
    ...
    <div class="dash-widget-sub" ${top ? `title="${_dashEscape(top.file || "")}"` : ""}>
      ${top ? `${_dashEscape(String(top.file).split("/").pop())} \xB7 ${top.count}` : "No data"}
    </div>
    ...
    if (size === "M") {
      ...
      const rows = sliced.length ? sliced.map((it, i) => _dashCouplingRow(it, i, imported, _dashT("dashCouplingImports"))).join("") : '<div class="dash-empty">No data</div>';
      container.innerHTML = `
<div class="dash-kpi-m" style="align-items:stretch">
  ...
  renderDetail(container, stats) {
    _dashCouplingRenderDetail(container, stats);
  }
});
