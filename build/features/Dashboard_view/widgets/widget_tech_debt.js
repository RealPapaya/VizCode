...
<div class="dash-kpi-l">
  ...
  renderDetail(container, stats) {
    ...
    let showDays = false;
    function _fmtVal() {
      return showDays ? (hours / HOURS_PER_DAY).toFixed(1) : hours.toFixed(1);
    }
    function _fmtSuffix() {
      return showDays ? "days" : "hours";
    }
    function _fmtSummary() {
      const top = nonZero.slice().sort((a, b) => Number(breakdown[b.key] || 0) - Number(breakdown[a.key] || 0))[0];
      return top ? `Largest category: ${_dashT(top.label)}. ${nonZero.length} issue types detected.` : `${nonZero.length} issue types detected.`;
    }
  ...
  <div class="dash-debt-detail-comp__stack">${heroSegments.map(
      (s) => `<span style="width:${s.pct}%;background:${s.col}" title="${_dashEscape(s.label)} ${s.pct}%"></span>`
    ).join("")}</div>
  <div class="dash-debt-detail-comp__legend">${heroSegments.map(
      (s) => `<button type="button" onclick="_dashOpenFileGroupDrilldown('Tech Debt: ${_dashEscape(s.label)}', _dashDebtCategoryFiles(${_dashJson(s.key)}, DATA.stats))">
    ...
    const recCards = nonZero.slice().sort((a, b) => Number(breakdown[b.key] || 0) - Number(breakdown[a.key] || 0)).map((d, i) => {
      const minutes = Number(breakdown[d.key] || 0);
