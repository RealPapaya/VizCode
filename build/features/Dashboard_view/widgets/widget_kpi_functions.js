...
_dashRegisterWidget({
  ...
  render(container, size, stats) {
    ...
    if (size === "S") {
      const topFiles = _dashAllFiles().filter((f) => (f.func_count || 0) > 0).sort((a, b) => (b.func_count || 0) - (a.func_count || 0)).slice(0, 3).map((f) => ({
...
${_dashKpiDetailGridHTML([
    ...
    function renderChart() {
      ...
      const rows = circular && modEntries.length > 5 ? modEntries.slice(0, 4).map((e) => ({ label: e[1], mods: [e[0]], count: e[2] })).concat([{
