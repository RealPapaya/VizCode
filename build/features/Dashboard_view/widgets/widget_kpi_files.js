...
<div class="dash-kpi-l">
  ...
  renderDetail(container, stats) {
    ...
    const sliceRows = langs.length > 5 ? langs.slice(0, 4).map(([ext, cnt]) => ({ label: `.${ext}`, exts: [ext], count: cnt })).concat([{
...
${_dashKpiDetailSectionHTML({
    ...
    function renderChart() {
      ...
      const rows = circular ? sliceRows : langs.slice(0, 12).map(([ext, cnt]) => ({ label: `.${ext}`, exts: [ext], count: cnt }));
