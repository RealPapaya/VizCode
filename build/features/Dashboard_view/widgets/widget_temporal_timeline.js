  ...
  _dashMkChart(canvas, type, {
    ...
    scales: {
      ...
      yCommits: {
        ...
        title: { display: true, text: _dashT("dashTemporalCommits"), color: _dashAccentTint(1) },
        beginAtZero: true
      },
      yLines: {
        position: "right",
...
function _dashChurnTimelinePeakWeek(buckets) {
  return (buckets || []).reduce((best, row) => Number(row.commits || 0) > Number(best.commits || 0) ? row : best, {});
}
...
_dashRegisterWidget({
  ...
  renderDetail(container, stats) {
    ...
    const summary = buckets.length ? `${_dashFmtExactNum(buckets.length)} weekly buckets${rangeLabel ? ` from ${_dashEscape(rangeLabel)}` : ""}. Peak week ${_dashEscape(peak.week_start || "n/a")} has ${_dashFmtExactNum(peak.commits || 0)} commits.` : _dashEscape(_dashT("dashTemporalEmpty"));
    container.innerHTML = `
