...
const _DASH_COMMIT_ACTIVITY_RANGES = [30, 60, 90, 180, 0];
function _dashCommitActivityInitialDays(stats) {
  const dates = (stats.commit_activity_daily || []).map((row) => String(row.date || "").trim()).filter(_dashHeatmapValidISO).sort();
...
function _dashCommitActivityRangeDays(container, stats) {
  if (container?.dataset.commitActivityDays != null) {
    return Math.max(0, Number(container.dataset.commitActivityDays) || 0);
  }
  const days = _dashCommitActivityInitialDays(stats);
  if (container) container.dataset.commitActivityDays = String(days);
  return days;
}
function _dashCommitActivityFilteredStats(stats, days) {
  if (!days) return stats;
  const dates = (stats.commit_activity_daily || []).map((row) => String(row.date || "").trim()).filter(_dashHeatmapValidISO).sort();
  const endStr = dates[dates.length - 1] || stats.window_end || stats.period_end;
  const end = _dashHeatmapParseISO(endStr);
  if (!end) return stats;
  const cutoff = _dashHeatmapISOStr(_dashHeatmapAddDays(end, -(days - 1)));
  return Object.assign({}, stats, {
    commit_activity_daily: (stats.commit_activity_daily || []).filter((row) => String(row.date || "") >= cutoff && String(row.date || "") <= endStr),
    window_start: cutoff,
    window_end: endStr,
    period_start: cutoff,
    period_end: endStr,
...
function _dashCommitActivityRangeHTML(activeDays) {
  return `
<span class="dash-commit-activity-ranges" role="group" aria-label="Commit activity date range">
  ${_DASH_COMMIT_ACTIVITY_RANGES.map((days) => {
    const label = days ? `${days}d` : "All";
    const active = days === activeDays ? " active" : "";
    return `<button type="button" class="dash-commit-activity-range${active}" data-commit-activity-days="${days}" aria-pressed="${days === activeDays}">${label}</button>`;
  }).join("")}
</span>`;
}
function _dashBindCommitActivityRanges(container, render) {
  container?.querySelectorAll("[data-commit-activity-days]").forEach((btn) => {
    btn.addEventListener("click", (event) => {
      event.stopPropagation();
      const days = Math.max(0, Number(btn.dataset.commitActivityDays) || 0);
      container.dataset.commitActivityDays = String(days);
      render();
    });
  });
}
...
<div class="dash-temporal-heatmap-legend">
  ...
  ${palette.slice(1).map(
    (c) => `<span class="dash-temporal-heatmap-swatch" style="background:${c}"></span>`
  ).join("")}
...
function _dashCommitActivityModel(stats) {
  ...
  const peak = dayValues.reduce((best, [date, d]) => d.commits > Number(best.commits || 0) ? { date, commits: d.commits, additions: d.additions, deletions: d.deletions } : best, {});
  const weekMap = /* @__PURE__ */ new Map();
  if (model) {
    model.dayMap.forEach((v, dateStr) => {
      const d = _dashHeatmapParseISO(dateStr);
...
function _dashCommitActivityFileRowsHTML(stats, limit, compact) {
  ...
  return rows.map((row, i) => {
    ...
    const meta = compact ? `${_dashFmtNum(commits)} ${_dashEscape(_dashT("dashTemporalCommits"))}` : `${_dashEscape(file)} &middot; +${_dashFmtNum(row.additions || 0)} / -${_dashFmtNum(row.deletions || 0)}`;
    return `
...
function _dashOpenCommitDayDrilldown(date) {
  ...
  const displayedCommitCount = needsCommitRows ? Math.max(...commits.map((c) => Number(c.aggregate_commits || 0)), commits.length) : commits.length;
...
<div class="dash-commit-drilldown-panel" role="dialog" aria-modal="true" aria-label="Commits on ${_dashEscape(day)}">
  ...
  overlay.querySelector("[data-commit-history-refresh]")?.addEventListener("click", (e) => _dashFetchCommitHistory(day, e.currentTarget));
...
function _dashAggregateCommitRowsForDay(day, stats) {
  ...
  return [{
    ...
    date: day,
    author: stats._commitHistoryRefreshFailed ? "Git history refresh failed; re-run analysis to restore per-commit rows" : "Re-run analysis to restore per-commit rows",
...
function _dashCommitHealthNotice(day, opts) {
  ...
  const note = needsCommitRows ? "This loaded dashboard does not have per-commit file rows yet." : "No Code Health snapshot exists for this date yet.";
  const hint = needsCommitRows ? "Refresh git history or re-run analysis to list individual commits." : "Run the normal Health Trend daily backfill or re-run analysis when you need a new health point.";
  return `
...
function _dashCommitDrillRowHTML(commit, impact, index, commitCount) {
  const sha = commit.aggregate ? "commit rows unavailable" : String(commit.short_sha || commit.sha || "").slice(0, 8) || "unknown";
  ...
  const detailHTML = commit.aggregate ? _dashCommitRowsUnavailableHTML(commit) : `${_dashCommitHealthBreakdownHTML(impact)}
...
function _dashCommitFilesHTML(commit) {
  ...
  const rows = files.map((row, i) => {
    ...
    const deltaHTML = hasLineDelta ? `<span class="dash-commit-file-row__delta"><b>+${_dashFmtExactNum(add)}</b><b>-${_dashFmtExactNum(del)}</b></span>` : `<span class="dash-commit-file-row__delta dash-commit-file-row__delta--meta">${_dashEscape(row.count != null ? `${row.count} commits` : "lines n/a")}</span>`;
    return `
<div class="dash-commit-file-row" data-clickable="true" data-tip="${_dashEscape(file)}"
  ...
  const capped = commit.files_capped ? `<div class="dash-commit-file-row__note">Showing ${_dashFmtExactNum(files.length)} of ${_dashFmtExactNum(fileCount)} changed files.</div>` : "";
  return rows + capped;
}
function _dashCommitHealthEntries(stats) {
  return (stats && stats.health_history || []).filter((e) => e && e.score != null).slice().sort((a, b) => {
...
function _dashCommitHealthImpact(commit, stats, dayCommitCount) {
  ...
  if (idx < 0) {
    ...
    const dateMatches = entries.map((entry2, i) => ({ entry: entry2, i })).filter(({ entry: entry2 }) => (entry2.date || (entry2.ts || "").slice(0, 10)) === date);
...
function _dashCommitEntryMatches(entry, commit) {
  ...
  const candidates = [entry.commit_full, entry.sha, entry.commit].map((v) => String(v || "").trim().toLowerCase()).filter((v) => v.length >= 7);
  return candidates.some((v) => sha && (sha === v || sha.startsWith(v) || v.startsWith(sha)) || short && (short === v || v.startsWith(short)));
}
...
function _dashCommitDayHealthHTML(impact, day) {
  ...
  const source = impact.matchType === "commit" ? "Exact commit snapshot for this day." : "Using the available Code Health snapshot for this date; commit rows share this day-level delta.";
  const scoreLine = impact.prevScore == null ? `${impact.score.toFixed(1)} baseline` : `${impact.prevScore.toFixed(1)} -> ${impact.score.toFixed(1)} (${_dashCommitDeltaText(impact.delta)})`;
  return `
...
function _dashMergeCommitHistory(payload) {
  ...
  keys.forEach((key) => {
    if (payload[key] !== void 0) DATA.stats[key] = payload[key];
  });
}
function _dashRenderCommitActivityWidget(container, size, stats) {
  const days = _dashCommitActivityRangeDays(container, stats);
  const filteredStats = _dashCommitActivityFilteredStats(stats, days);
  if (size === "S") {
    const rows = filteredStats.commit_activity_daily || [];
...
<div class="dash-kpi-s">
  <div class="dash-kpi-s-body">
    ...
    <div class="dash-widget-stat">${_dashFmtNum(rows.reduce((sum, row) => sum + Number(row.commits || 0), 0))}</div>
  ...
  _dashRenderTemporalHeatmap(container, filteredStats);
  container.querySelector(".dash-card-title")?.insertAdjacentHTML(
    "beforeend",
    _dashCommitActivityRangeHTML(days)
  );
  if (size === "L") {
    container.classList.add("dash-commit-activity-l");
    _dashHeatmapAppendStats(container, filteredStats);
    _dashHeatmapAppendChurnFiles(container, stats);
  }
  _dashBindCommitActivityRanges(container, () => _dashRenderCommitActivityWidget(container, size, stats));
}
function _dashRenderCommitActivityDetail(container, stats) {
  const days = _dashCommitActivityRangeDays(container, stats);
  const filteredStats = _dashCommitActivityFilteredStats(stats, days);
  const activity = _dashCommitActivityModel(filteredStats);
  const totalCommits = activity.model ? activity.model.totalCommits : 0;
  const peakLabel = activity.peak.date ? `Peak day ${_dashEscape(activity.peak.date)} with ${_dashFmtExactNum(activity.peak.commits || 0)} commits.` : "No peak day in this window.";
  const rangeLabel = [filteredStats.period_start, filteredStats.period_end].filter(Boolean).join(" to ");
...
<div class="dash-commit-activity-detail">
  ...
  _dashBindCommitActivityRanges(container, () => _dashRenderCommitActivityDetail(container, stats));
}
_dashRegisterWidget({
  id: "commit_heatmap",
  ...
  render(container, size, stats) {
    _dashRenderCommitActivityWidget(container, size, stats);
  },
  renderDetail(container, stats) {
    _dashRenderCommitActivityDetail(container, stats);
  }
});
