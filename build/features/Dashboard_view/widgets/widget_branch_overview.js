...
function _dashBranchShowError(container, msg) {
  const body = container.querySelector(".dash-bf-body") || container.querySelector("#dash-branch-list-l") || container.querySelector("#dash-branch-diff-panel") || container;
...
<div class="dash-list dash-bf-body" id="dash-branch-list-m"></div>`;
  _dashBranchLoad(container, stats, (data) => {
    ...
    if (!branches.length) {
      el.innerHTML = `<div class="dash-empty">${_dashEscape(_dashT("dashBranchNoData"))}</div>`;
      return;
    }
...
<div class="dash-branch-row dash-list-row" data-branch="${name}"
  ...
  <div style="display:flex;align-items:center;gap:5px;opacity:0.65;font-size:0.7rem">
    <span>${author}</span>
    <span style="opacity:0.5">\xB7</span>
    <span>${date}</span>
...
function _dashBranchShowDiff(panel, b, base, stats) {
  ...
  fetch(url).then((r) => r.json()).then((diff) => {
    if (diff.error) {
      panel.innerHTML = `<div class="dash-empty">${_dashEscape(diff.error)}</div>`;
      return;
    }
    _dashBranchRenderDiff(panel, b, diff, stats);
  }).catch(() => {
    panel.innerHTML = `<div class="dash-empty">${_dashEscape(_dashT("dashBranchNoData"))}</div>`;
  });
}
