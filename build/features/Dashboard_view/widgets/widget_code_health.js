...
_dashRegisterWidget({
  ...
  render(container, size, stats) {
    ...
    const statusKey = score >= _DASH_HEALTH_BANDS.amber ? "dashHealthGood" : score >= _DASH_HEALTH_BANDS.red ? "dashHealthFair" : "dashHealthPoor";
    const pct = Math.max(0, Math.min(1, score / 10));
    const fillLen = (pct * _DASH_HEALTH_TRACK_LEN).toFixed(2);
    const gapLen = _DASH_HEALTH_TRACK_LEN.toFixed(2);
    if (size === "S") {
      const worst = _DASH_HEALTH_SUBSCORES.map((s) => ({ label: _dashT(s.label).split(" ").pop(), value: Number(breakdown[s.key] || 0).toFixed(1), key: s.key })).sort((a, b) => Number(a.value) - Number(b.value)).slice(0, 3).map((s) => ({
        label: s.label,
        value: s.value,
