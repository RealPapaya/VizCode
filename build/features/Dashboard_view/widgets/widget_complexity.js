function _dashComplexityBucketFunctions(range) {
  ...
  const fromIndex = Object.values(window.DATA && DATA.symbol_index || {}).filter((sym) => (sym.kind === "function" || sym.kind === "method") && sym.complexity != null).filter((sym) => Number(sym.complexity) >= lo && Number(sym.complexity) <= hi).sort((a, b) => b.complexity - a.complexity).map((sym) => ({ file: sym.file, name: sym.name, value: `complexity ${sym.complexity}` }));
  if (fromIndex.length) return fromIndex;
  return (DATA.stats.complexity_top_offenders || []).filter((sym) => Number(sym.complexity || 0) >= lo && Number(sym.complexity || 0) <= hi).map((sym) => ({ file: sym.file, name: sym.name, value: `complexity ${sym.complexity}` }));
}
_dashRegisterWidget({
  id: "complexity",
...
<div style="height:100%;box-sizing:border-box;display:flex;flex-direction:column;gap:var(--space-2);">
  ...
  renderDetail(container, stats) {
    ...
    const heroVisual = (() => {
      ...
      return `<div class="dash-cyclo-detail-hist">${buckets.map((b, i) => {
    ...
    const summaryText = topName ? `Top offender: ${topName} (complexity ${topScore}) in ${topFile}. ${top.length} functions reported.` : `${top.length} functions reported.`;
    container.innerHTML = `
