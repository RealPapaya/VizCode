...
<div class="dash-arch-panel">
  <div class="dash-arch-panel-header">
    <div class="dash-arch-panel-title-block">
      ...
      <div class="dash-arch-panel-sub">${entryCount} ${_dashEscape(_dashT("dashIssuesEntry"))} \xB7 ${isoCount} ${_dashEscape(_dashT("dashIssuesIsolated"))}</div>
    </div>
  </div>
  <div class="dash-arch-panel-body">
...
<div class="dash-entry-detail-hero-stats">
  ...
  <div class="dash-entry-detail-hero-stat">
    ...
    const summaryText = entryCount === 0 ? "No entry points detected in this codebase." : `${entryCount} file${entryCount !== 1 ? "s" : ""} not imported anywhere${isoCount > 0 ? `, ${isoCount} fully isolated` : ""}.`;
    const entryRows = entries.length ? entries.map((file, i) => {
      const short = String(file).split("/").pop();
      const fileJson = _dashJson(file);
    ...
    }).join("") : `<div class="dash-empty">${_dashEscape(_dashT("dashEntryPointsEmpty"))}</div>`;
    const isoRows = isolated.length ? isolated.map((file, i) => {
      const short = String(file).split("/").pop();
      const fileJson = _dashJson(file);
      return `<div class="dash-kpi-detail-row" data-clickable="true" title="${_dashEscape(file)}"
              onclick="_dashGoToGraphFile(${fileJson}, null)">
...
${_dashKpiDetailSectionHTML({
      ...
      body: _dashKpiDetailStatsHTML([
        ...
        { value: String(isoCount), label: "isolated files", color: isoCount > 0 ? "var(--muted)" : void 0 }
      ])
    })}
${_dashKpiDetailSectionHTML({
