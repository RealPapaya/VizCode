...
function _dashRenderStructure(container, stats, options) {
  ...
  const layoutStyle = isDetail ? "box-sizing:border-box;display:flex;flex-direction:column;gap:var(--space-3);" : "height:100%;box-sizing:border-box;display:flex;flex-direction:column;gap:var(--space-3);overflow:hidden;";
  const chartGridStyle = isDetail ? "" : "flex:1;min-height:0;overflow:hidden;";
  const chartCardStyle = isDetail ? "" : "display:flex;flex-direction:column;overflow:hidden;min-height:0;";
  const chartWrapStyle = isDetail ? "" : "flex:1;min-height:0;";
  const treemapStyle = isDetail ? "" : "flex:0 0 auto;max-height:35%;overflow:hidden;";
  ...
  const layoutOpen = isDetail ? "" : `<div class="dash-arch-panel dash-structure-panel">
...
function _dashChartFileTypes(stats, scope) {
  ...
  const rows = isCircular ? _dashStructureRows(sorted, (key) => key.replace("_", " ")) : sorted.map(([key, value]) => ({ key, keys: [key], label: key.replace("_", " "), value }));
  ...
  const strokes = fills.map(() => _dashAccentTint(1));
  _dashMkChart(canvas, type, {
    ...
    indexAxis: type === "bar" ? "y" : void 0,
    plugins: {
      legend: isCircular ? { position: isDetail ? "bottom" : "right", labels: { boxWidth: 10, padding: isDetail ? 8 : 12 } } : { display: false }
    },
    scales: type === "bar" ? {
...
function _dashChartLanguageDist(stats, scope) {
  ...
  const rows = isPie ? _dashStructureRows(sorted, (ext) => ext || "unknown") : sorted.map(([key, value]) => ({ key, keys: [key], label: key || "unknown", value }));
  const labels = rows.map((row) => row.label);
  const vals = rows.map((row) => row.value);
  ...
  _dashMkChart(canvas, type, {
    ...
    plugins: {
      legend: isPie ? { position: isDetail ? "bottom" : "right", labels: { boxWidth: 10, padding: isDetail ? 8 : 10 } } : { display: false }
    },
    scales: type === "bar" ? {
      x: { grid: { color: _dashBorderTint(0.6) } },
...
function _dashBuildTreemap(scope) {
  ...
  }).filter((m) => m.size > 0).sort((a, b) => b.size - a.size);
  if (!modules.length) {
  ...
  el.innerHTML = modules.map((m) => {
    ...
    const click = m.firstFile ? ` data-clickable="true" onclick="_dashOpenFileGroupDrilldown('Module ${_dashEscape(m.label)}', _dashFilesByModule(${_dashJson(m.id)}))"` : "";
    return `
