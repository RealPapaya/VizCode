...
<div class="dash-kpi-s">
  <div class="dash-kpi-s-body">
    ...
    if (size === "M") {
      const fillPct2 = Math.max(0, Math.min(100, pct));
...
<div class="dash-kpi-m">
  ...
  <div class="dash-kpi-m-right">
    ...
    <div class="dash-kpi-bar-row" style="margin-top:8px;">
      <div class="dash-kpi-bar-track"><div class="dash-kpi-bar-fill" style="width:${fillPct2}%;background:${color}"></div></div>
  ...
  <span class="dash-kpi-bar-val">${occ.length}\xD7</span>
</div>`;
    }).join("");
    container.innerHTML = `
<div class="dash-kpi-l">
  ...
  renderDetail(container, stats) {
    ...
    blocks.forEach((blk) => (blk.occurrences || []).forEach((o) => {
      if (o.file) affectedFiles.add(o.file);
    }));
    const heroVisual = `
    ...
    const blockRows = blocks.map((blk, i) => {
      ...
      return `<div class="dash-kpi-detail-row dash-dup-detail-block-row" data-clickable="true"
                ...
                <span class="dash-kpi-detail-row__name">
                    ${_dashEscape(fileShort)}<span class="dash-dup-detail-meta">:${first.line || "?"} \xB7 ${_dashEscape(blk.sample ? blk.sample.trim().slice(0, 60) : "")}</span>
                </span>
                <span class="dash-kpi-detail-row__value">${occ.length}\xD7</span>
            </div>`;
    }).join("") || `<div class="dash-empty">\u2705 ${_dashEscape(_dashT("dashDuplicationNone"))}</div>`;
    const summaryText = blocks.length ? `${blocks.length} duplicated block${blocks.length !== 1 ? "s" : ""} detected across ${affectedFiles.size} file${affectedFiles.size !== 1 ? "s" : ""}. ${label.charAt(0).toUpperCase() + label.slice(1)} duplication level.` : `No duplicated blocks detected. Codebase is clean.`;
    container.innerHTML = `
