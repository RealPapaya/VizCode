...
function _dashRenderHealthDetail(container, history) {
  ...
  const deltaCol = delta == null ? "inherit" : delta > 0 ? "var(--status-good)" : delta < 0 ? "var(--status-bad)" : "var(--muted)";
  const subKeys = ["complexity", "coupling", "dead_code", "duplication", "cohesion"];
  const subLabels = [
    _dashT("dashHealthComplexity"),
    _dashT("dashHealthCoupling"),
    _dashT("dashHealthDeadCode"),
    _dashT("dashHealthDuplication"),
...
${_dashReportSection({ title: "Backfill", body: bfHtml })}`;
  if (history.length >= 2) {
    requestAnimationFrame(() => {
      ...
      _dashDrawHealthTrendLarge(
        canvas,
        recent,
        _dashHealthTrendLabels(recent),
...
function _dashBfStart(mode, btn) {
  ...
  const days = mode === "commits" ? Math.max(7, Math.min(365, Number(stats.window_days || 90))) : 90;
  const body = JSON.stringify({ mode, days, job_id: jobId });
  fetch("/api/health-backfill", { method: "POST", headers: { "Content-Type": "application/json" }, body }).then((r) => r.json()).then((data) => {
    if (data.error) {
      _dashBfShowError(data.error);
      return;
    }
    _dashBfState.token = data.token;
    _dashBfState.mode = mode;
    _dashBfState.finished = false;
    const panel = typeof _dashGetDetailBody === "function" ? _dashGetDetailBody() : document.getElementById("dash-detail-body");
...
function _dashBfPoll(bfEl, detailContainer) {
  if (!_dashBfState.token) {
    clearInterval(_dashBfState.timer);
    return;
  }
  fetch(`/api/health-backfill-status?token=${_dashBfState.token}`).then((r) => r.json()).then((prog) => {
    const bar = bfEl.querySelector("#dash-ht-bf-bar");
    const label = bfEl.querySelector("#dash-ht-bf-label");
    const pct = prog.total > 0 ? Math.round(prog.done / prog.total * 100) : 0;
    if (bar) bar.style.width = pct + "%";
    if (label) label.textContent = prog.current_date ? `${prog.done}/${prog.total} \xB7 ${prog.current_date} (${prog.current_sha})` : `${prog.skipped || 0} already cached`;
    if (prog.finished) {
      clearInterval(_dashBfState.timer);
...
function _dashBfRefresh(btn) {
  ...
  const body = document.getElementById("dash-detail-report-details") || (typeof _dashGetDetailBody === "function" ? _dashGetDetailBody() : document.getElementById("dash-detail-body"));
