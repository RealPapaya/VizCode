(function() {
  "use strict";
  const _DASH_AI_SUPPORTED = /* @__PURE__ */ new Set([
    "dead_code",
    "circular_deps",
    "issues",
    "duplication",
  ...
  const _dashAiJobs = /* @__PURE__ */ new Map();
  ...
  function _dashAiFinalizeItems(items) {
    return (items || []).map(_dashAiItem).filter((item) => item.files.length || item.symbols.length || item.evidence.length || item.summary).sort((a, b) => {
  ...
  function _dashAiSecurityExtractor(stats) {
    ...
    const all = [].concat((f.by_severity || {}).high || []).concat((f.by_severity || {}).medium || []).concat((f.by_severity || {}).low || []);
  ...
  function _dashAiWidgetLabel(widgetId) {
    const registry = typeof _dashWidgetRegistry !== "undefined" ? _dashWidgetRegistry : window._dashWidgetRegistry || {};
  ...
  function _dashAiSyncIndicators() {
    ...
    document.querySelectorAll("#dashboard-bento .dash-widget").forEach((widgetEl) => {
      ...
      hint.textContent = isBusy ? _dashAiT("dashAiPromptWidgetGenerating", "AI prompt generating") : _dashAiT("dashAiPromptWidgetUnread", "AI prompt ready");
  ...
  async function _dashAiLoadConfig() {
    ...
    try {
      ...
      state.status = state.ready ? `${_dashAiT("dashAiPromptReady", "AI ready")}: ${_dashAiConfigLabel(cfg || {})}` : _dashAiT("dashAiPromptAiMissingTitle", "AI is not configured");
    } catch (err) {
      if (!_dashAiJobAlive(state)) return;
      state.config = {};
      state.ready = false;
  ...
  function _dashAiBuildPrompt() {
    ...
    const context = {
      ...
      scope: state.scope,
      scope_intent: state.scope === "single" ? "Verify one selected finding and produce a targeted remediation prompt." : state.scope === "multiple" ? "Verify the selected findings in priority order and produce a bounded remediation prompt." : "Verify the whole widget signal and produce a triage/remediation strategy.",
  ...
  async function _dashAiGenerate() {
    ...
    try {
      ...
      state.status = state.parsed.prompt ? _dashAiT("dashAiPromptReadyResult", "Prompt ready") : _dashAiT("dashAiPromptRawResponse", "AI response received");
...
<div class="dash-ai-check-list">
  ...
  function _dashAiResultHtml(state) {
    ...
    const streamingPreview = state.busy && state.response ? `<pre class="dash-ai-stream-preview">${_dashAiEsc(state.response)}</pre>` : "";
    const summary = parsed?.summary ? `<section class="dash-ai-result-section"><h4>${_dashAiEsc(_dashAiT("dashAiPromptSummary", "Verification Summary"))}</h4><pre>${_dashAiEsc(parsed.summary)}</pre></section>` : "";
    const prompt = parsed?.prompt ? `<section class="dash-ai-result-section"><h4>${_dashAiEsc(_dashAiT("dashAiPromptAgentPrompt", "Agent Prompt"))}</h4><textarea readonly>${_dashAiEsc(parsed.prompt)}</textarea></section>` : "";
    const rawFallback = parsed && !parsed.prompt && parsed.raw ? `<section class="dash-ai-result-section"><h4>${_dashAiEsc(_dashAiT("dashAiPromptRawResponse", "Raw AI Response"))}</h4><pre>${_dashAiEsc(parsed.raw)}</pre></section>` : "";
  ...
  function _dashAiRenderModal() {
    ...
    const generateLabel = state.response || state.parsed ? _dashAiT("dashAiPromptRegenerate", "Regenerate") : _dashAiT("dashAiPromptGenerate", "Generate");
    const status = state.status || (state.configLoading ? _dashAiT("dashAiPromptChecking", "Checking AI settings...") : "");
    overlay.innerHTML = `
<div class="dash-ai-modal" role="dialog" aria-modal="true" aria-labelledby="dash-ai-modal-title">
  <div class="dash-ai-modal-head">
    <div>
      ...
      <p>${_dashAiEsc(state.widgetLabel)} \xB7 ${state.items.length} ${_dashAiEsc(_dashAiT("dashAiPromptFindings", "findings"))}</p>
    </div>
    <button class="dash-detail-close" type="button" data-ai-close aria-label="${_dashAiEsc(_dashAiT("dashAiPromptClose", "Close"))}">\xD7</button>
  </div>
  ...
  window.addEventListener("vizAiConfigChanged", (event) => {
    ...
    _dashAiState.error = "";
    _dashAiState.status = _dashAiState.ready ? `${_dashAiT("dashAiPromptReady", "AI ready")}: ${_dashAiConfigLabel(_dashAiState.config)}` : _dashAiT("dashAiPromptAiMissingTitle", "AI is not configured");
    _dashAiRenderModal();
  });
})();
