function initCy() {
  ...
  cy = cytoscape({
    ...
    motionBlur: false,
    // avoids dark-frame accumulation
    pixelRatio: Math.min(window.devicePixelRatio || 1, 1.5)
    // cap HiDPI render cost
  });
  window.cy = cy;
  ...
  cy.on("mousedown", (e) => {
    ...
    _bgPointerDownPos = _bgPointerDown && e.renderedPosition ? { x: e.renderedPosition.x, y: e.renderedPosition.y } : null;
  });
  cy.on("mousemove", (e) => {
  ...
  cy.on("mouseover", "node", (e) => {
    const node = e.target;
    try {
      document.dispatchEvent(new CustomEvent("vizNodeHover", { detail: { nodeId: node.id(), enter: true } }));
    } catch (_) {
    }
    if (_hoveredNodeClearTimer) {
      clearTimeout(_hoveredNodeClearTimer);
  ...
  cy.on("mouseout", "node", (e) => {
    ...
    const node = e.target;
    try {
      document.dispatchEvent(new CustomEvent("vizNodeHover", { detail: { nodeId: node.id(), enter: false } }));
    } catch (_) {
    }
...
let _lastTappedEdge = null;
let _hoveredNodeClearTimer = null;
let _labelStyledNodes = null;
let _selectedLabelNode = null;
let _neighborLabelNodes = null;
...
function _getHighlightedLabelFontSize(node, targetScreenPx, zoom) {
  const safeZoom = Math.max(zoom || 1, 1e-4);
...
function _spatialPlacementNow() {
  ...
  cy.nodes().forEach((n) => {
    ...
    const alwaysShow = n.data("lvl") === 0 || n.isParent() || !n.data("simple") || n.hasClass("selected-label") || n.hasClass("neighbor-label") || n.hasClass("node-hovered");
    ...
    items.push({
      ...
      bb: {
        ...
        y1: pos.y - nodeR,
        x2: pos.x + estW / 2,
        y2: pos.y + nodeR + labelH + 6
      }
    });
  });
  items.sort((a, b) => a.alwaysShow !== b.alwaysShow ? a.alwaysShow ? -1 : 1 : b.degree - a.degree);
  const CELL = 80;
  const grid = /* @__PURE__ */ new Map();
  function _cellKey(cx, cy2) {
    return cx << 16 ^ cy2;
  }
  ...
  function overlaps(bb) {
    ...
    for (let gx = cx0; gx <= cx1; gx++) {
      for (let gy = cy0; gy <= cy1; gy++) {
        ...
        if (!arr) continue;
        for (const p of arr) {
          if (bb.x1 - PAD < p.x2 && bb.x2 + PAD > p.x1 && bb.y1 - PAD < p.y2 && bb.y2 + PAD > p.y1) return true;
        }
      }
    }
    return false;
