...
async function _galaxyWaitForBackgroundIdle(token, quietMs = 700) {
    ...
    if (typeof window.requestIdleCallback === 'function') {
        await new Promise<void>(resolve => window.requestIdleCallback(() => resolve(), { timeout: 180 }));
    } else {
...
function _gEstimateTotalNodes(data) {
    const D: any = data || window.DATA || {};
    let files = 0;
    for (const arr of Object.values(D.files_by_module || {}) as any[]) files += arr.length;
    for (const arr of Object.values(D.other_files_by_module || {}) as any[]) files += arr.length;
...
function _gBuildPathTree(data) {
    ...
    for (const sym of Object.values(D.symbol_index || {}) as any[]) {
        const p = _gNormPath(sym.file);
        fileSymbols.set(p, (fileSymbols.get(p) || 0) + 1);
    }
    const allFiles = [];
    for (const arr of Object.values(D.files_by_module || {}) as any[]) allFiles.push(...arr);
    for (const arr of Object.values(D.other_files_by_module || {}) as any[]) allFiles.push(...arr);
...
function _galaxyInitSigma() {
    ...
    if (edgeCurveProg) {
        (sigmaSettings as any).edgeProgramClasses = { curved: edgeCurveProg, line: SigmaClass.rendering.EdgeLineProgram };
    }
