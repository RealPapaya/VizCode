...
// Phase 2 orchestrator. Lays out three sub-cards (hotspot / coupling /
// timeline) and delegates rendering to the per-card sub-widgets. Returns
// early when there's no git history or no commits in the analysed window.
...
<div class="dash-temporal-period">
  ...
  <span class="dash-temporal-period-range">
    ${_dashEscape(stats.period_start)} → ${_dashEscape(stats.period_end)}
  </span>
  <span class="dash-temporal-period-meta">
    ${stats.commits_analyzed} ${_dashEscape(_dashT('dashTemporalCommits'))} · ${stats.window_days}d
  </span>
</div>`;
