...
//   { score, total, counts:{high,medium,low}, top_issues, by_severity, by_rule, trend }
//
// Size-responsive:
//   S — score number + top 3 issue titles
//   M — score gauge + 5–8 issue list
//   L — score + High/Medium/Low columns
//   Detail panel — full list + per-run trend sparkline
...
function _dashSecScoreColor(score) {
    // Reuse the dashboard's existing 0–10 health palette if available;
    // fall back to a simple traffic-light otherwise.
...
function _dashSecRenderSmall(container, f) {
    ...
    const top3  = (f.top_issues || []).slice(0, 3).map(i =>
        `<div style="display:flex;align-items:center;gap:5px;font-size:0.7rem;opacity:0.7">
...
function _dashSecRenderLarge(container, f) {
    ...
    const col = (sev, label) => {
        ...
        const rows  = items.length
            ? items.map(i => _dashSecIssueRow(i, { showCode: false })).join('')
            : `<div class="dash-empty" style="opacity:0.4;font-size:0.72rem;text-align:center;padding:8px">—</div>`;
...
<div class="dash-list-row" style="padding:6px 8px">
  ...
  <span style="font-size:0.72rem;opacity:0.6">${r.count} · ${r.files.length} files</span>
</div>`).join('');
